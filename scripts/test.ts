...
async function getSweCovidData() {
  ...
  const veckodata_rikiet = sheets[8];
  ...
  for (const row of veckodata_region) {
    ...
    data[row.Region]!.Kum_antal_fall.push(row.Kum_antal_fall);
    data[row.Region]!.Kum_fall_100000inv.push(row.Kum_fall_100000inv);
    data[row.Region]!.dateYearWeek.push(`${row.år}-${row.veckonummer}`);
  }
